...
| `src/gui/screen/mainMenu/online/OnlineSetupScreen.ts` | Entry screen only: Player Info + browsable room list, "Create Room"/"Back" sidebar. Owns the long-lived session objects (`LanMeshSession`, `LanRoomSession`, `OnlineRoomSession`, `PregameController`, `ChatHistory`) and hands them off to `OnlineRoomScreen` by reference on success. "Create Room" opens a native dialog (room name, max players, optional password) instead of forcing a `MapSelScreen` detour first. Joining opens a native password prompt (flagged via `passwordProtected` in the room list) instead of joining immediately. See §11.4 for why this is two screens, not one. |
| `src/gui/screen/mainMenu/online/OnlineRoomScreen.ts`, `component/OnlineRoom.tsx` | The waiting-room screen, pushed on top of `OnlineSetupScreen` once a room exists. Start Game/Change Map/Ready/Leave Room sidebar, launch handling, and the Change-Map sub-push all live here. Reuses `LanRoomSession`, `PregameController`, `LobbyForm`, `LanMatchSession`, `LanLockstepTurnManager` **completely unchanged** — once the mesh is formed, control hands off to the same code LAN play uses. "Change Map" applies the result to the already-created room (`applyHostPregameSnapshot`) rather than creating a second one. "Leave Room" pops back to `OnlineSetupScreen`. |
| `src/gui/screen/mainMenu/online/component/OnlineRoomList.tsx`, `CreateRoomForm.tsx`, `JoinRoomForm.tsx` | Room list panel; form content rendered inside `messageBoxApi.show(...)`'s native `Dialog` chrome (see §11.4) for Create/Join. |
...
**Future consideration — server-authoritative lobby state:** Colyseus could in principle own lobby state directly (`Slot[]`, `ready`, `gameOpts`, `mapName` as `@type()` schema on `MatchmakingState`, mutated via room message handlers) instead of `LanRoomSession` broadcasting it peer-to-peer over the mesh. That would remove the full-mesh requirement for the *lobby* phase (clients would only need a WS connection to the server, not a working P2P path to every other player) and eliminate host-migration logic entirely, since the server would be the single source of truth instead of whichever peer is "host."

The cost: it gives up this pass's central efficiency win — `LanRoomSession`/`PregameController`/`LobbyForm` are reused **completely unmodified** from LAN play (§11.1). Moving lobby state server-side means forking that into a second, server-authoritative implementation (`PregameController`'s mutations become room message handlers; `LobbyForm` subscribes to `state.onChange` instead of `LanMeshSession` broadcasts) — two parallel lobby code paths (LAN P2P vs. Online server-authoritative) instead of one shared one. It also wouldn't touch match execution itself: this is a deterministic-lockstep RTS, so gameplay still runs peer-to-peer over the WebRTC mesh via `LanLockstepTurnManager` regardless of who owns lobby state — Colyseus taking over the lobby doesn't extend to the match.

Not recommended unless full-mesh/NAT reliability for the lobby specifically becomes a measured real-world problem — the actual connectivity issues hit so far (STUN/DNS latency, §11's ICE-gathering fix) were about mesh *formation speed*, not the mesh model itself. If mesh reliability does become the blocker, TURN (already flagged above) is a much smaller, more targeted fix than forking the lobby state model.
...
### 11.4 Screen architecture: how to implement a new menu screen

The base `Controller` (`src/gui/screen/Controller.ts`) is a simple stack machine: `screens: Map<screenType, Screen>`, a `screenStack`, and one `currentScreen`. A `Screen` implements `onEnter`/`onLeave`/`onStack?`/`onUnstack?`. `pushScreen(type, params)` calls `onStack()` on whatever's current (demoting it onto the stack) then `onEnter(params)` on the new screen; `popScreen(params)` calls `onLeave()` on the current screen then `onUnstack(params)` on whatever's revealed underneath. There is **no built-in transition animation** in `Controller`/`MainMenuController` itself — what reads as a "transition" is really the sidebar being hidden then rebuilt (`hideSidebarButtons()`/`showSidebarButtons()`), the main video toggling, and the per-screen music restarting, all driven by each screen's own `onEnter`/`onLeave`. An internal React state flip inside one screen's component (no `pushScreen` call) gets none of that — it just re-renders in place.

**When to split "one screen with an internal mode" into two real screens:** if the two modes should feel like separate places (distinct sidebar buttons, their own "Back"/"Leave" affordance, a fade/rebuild instead of an instant swap), make them two `Screen` instances connected by `pushScreen`/`popScreen`, not a boolean prop threaded through one component. This is what `OnlineSetupScreen` → `OnlineRoomScreen` became in this pass (previously one screen toggling between an "entry" and "waiting" render mode via `refreshView()`, matching how `LanSetupScreen` still does it today — LAN was intentionally left as-is, only Online was split, so the two screens now differ in this respect).

**Two ways to register a screen with the controller:**
1. **Static, main-menu-reachable screens** (`Home`, `MapSelection`, `LanSetup`, `OnlineSetup`, …): add the enum to `MainMenuScreenType` in `ScreenType.ts`, register the class in `Gui.ts`'s `subScreens` map, and add a construction branch in `MainMenuRootScreen.createScreen()` that resolves its dependencies (`Rules`, `MapFileLoader`, `Config`, …) fresh from scratch. `MainMenuRootScreen.onEnter()` iterates this map and calls `controller.addScreen(type, screen)` for all of them up front, every time the main menu is (re)entered — so these screens are stateless from the router's point of view; they can't carry live objects into their constructor that need to survive across menu sessions.
2. **Lazily-registered screens that must share live state with their parent** (`OnlineRoomScreen`): don't route through `MainMenuRootScreen.createScreen()` at all. Instead, the owning screen constructs it once (guarded by a field, e.g. `private roomScreen?: OnlineRoomScreen`), calls `.setController(this.controller)`, and registers it with `this.controller.addScreen(type, screen)` on first use — `Controller.addScreen` is just a `Map.set`, callable at any time, not only during the initial subScreens loop. The live objects it needs (`LanMeshSession`, `LanRoomSession`, …) are passed through `pushScreen(type, params)` on every push and read fresh in `onEnter(params)`, rather than injected once via the constructor — this avoids stale references if the owning screen ever recreates one of those objects (e.g. a fresh `PregameController` after leaving a room).

**Pitfall hit while doing this split:** a screen's own cleanup code (e.g. a `finally` block after an `await` that might `pushScreen` away) must not unconditionally touch controller-level shared state like the sidebar afterward — `this.controller` is one shared `MainMenuController` instance, and it has no idea "this screen is no longer current" unless you track that yourself. `OnlineSetupScreen.finishCreateRoom()`/`handleJoinRoom()` originally did `finally { this.busy = false; this.refreshSidebarButtons(); }` — once `enterRoomScreen()` inside the `try` succeeded and pushed `OnlineRoomScreen`, that `finally` still ran and silently overwrote `OnlineRoomScreen`'s freshly-set sidebar buttons with the entry screen's stale "Create Room"/"Back". Fixed with an `enteredRoom` flag so the entry screen only touches the sidebar in the finally block when it's still actually the current screen.

**Dialog pattern:** use the existing `MessageBoxApi.show(message, buttons, options)` (`src/gui/component/MessageBoxApi.tsx`) for any modal, rather than building a bespoke overlay — it renders through the same `Dialog` component (`src/gui/component/Dialog.tsx`) as every other in-game message box, so it automatically gets the classic `.message-box`/`.prompt-box` chrome (fixed-size background art, red-bordered `.dialog-button` footer) instead of a generically-styled `<div>` overlay. `message` accepts a real React node — `React.createElement(SomeForm, {...})`, not the engine's separate scene-graph `jsx()` helper — and `buttons` is a `ButtonConfig[]` whose `onClick` fires *after* the dialog auto-disposes on click. Because the buttons live outside the form component's own React state, read the current field values via a mutable ref object (`{ current: {...} }`) that the form's `onChange` handlers write into directly and the button's `onClick` closure reads at click time — see `CreateRoomForm.tsx`/`JoinRoomForm.tsx` and `OnlineSetupScreen.openCreateRoomDialog()`/`openJoinRoomDialog()`. Also: `MessageBoxApi.show()`'s `buttons` argument is not optional in practice — passing only a `message` renders **zero** footer buttons (fine for a transient "please wait" state you'll replace yourself, a real bug for anything terminal like an error).
